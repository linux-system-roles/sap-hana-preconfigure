...
ansible-playbook sap-hana-preconfigure-sapnote-2777782-04-test.yml -l ${MANAGED_NODE} --check
RC=$?
echo "RC=${RC}"
if [[ ${RC} -ne 0 ]]; then
   exit ${RC}
...
ansible-playbook sap-hana-preconfigure-sapnote-2777782-04-test.yml -l ${MANAGED_NODE}
RC=$?
echo "RC=${RC}"
if [[ ${RC} -ne 0 ]]; then
   exit ${RC}
...
ansible-playbook sap-hana-preconfigure-sapnote-2777782-04-test.yml -l ${MANAGED_NODE} -e "{'sap_hana_preconfigure_assert': yes}"
RC=$?
echo "RC=${RC}"
if [[ ${RC} -ne 0 ]]; then
   exit ${RC}
